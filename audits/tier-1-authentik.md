...
|---|---|---|
| 6-i — Authentik recorded `10.42.0.1` (CNI bridge) as the source IP of every request, breaking IP reputation and audit log forensics | **Open (re-investigating)** | PR #52 (`externalTrafficPolicy: Local`) + PR #53 (`AUTHENTIK_LISTEN__TRUSTED_PROXY_CIDRS`) did not deliver real client IPs end-to-end through k3s's bundled LoadBalancer; root cause under investigation. Same scope still applies to Pi-hole, Jellyfin, Coder, Uptime Kuma audit logs |
