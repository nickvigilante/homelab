...
| 6-ii — Traefik passes through arbitrary upstream headers (e.g. `X-Forwarded-User`) that a misbehaving downstream could trust | **Resolved** | **PR #64** — default Middleware on the `websecure` entryPoint strips `X-Forwarded-User`, `Remote-User`, and `X-Authentik-*` headers on every HTTPS route |
